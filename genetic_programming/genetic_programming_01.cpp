...
class LexLessThan {
  public:
    bool operator() (const LinkedBinaryTree& a, const LinkedBinaryTree& b) {
      if (abs(a.getScore() - b.getScore()) < 0.005) {
        return a.size() * a.getSteps() > b.size() * b.getSteps(); // bigger size means its worst
      } else {
        return a.getScore() < b.getScore(); // bigger score means its better
      }
    }
};
...
void LinkedBinaryTree::deleteSubtreeMutator(mt19937& rng) {
  ...
  // position list reutrns elements in preorder traversal
  PositionList pl = positions();
  int numnodes = pl.size();

  // selects a random node to delete form the tree
  int chosenindex = randInt(rng, 1, numnodes-1);

  Position chosennode = pl[chosenindex]; // position of node that is chosen to be deleted

  // getting list of nodes belonging to the subtree
  PositionList pl_delete;
  preorder(chosennode.v, pl_delete);
  Position par = chosennode.parent();

  if (par.left().v == chosennode.v) {
    par.setLeft(NULL);
  } else {
    par.setRight(NULL);
  }

  for (auto &i : pl_delete) {
    delete i.v;
  }
}

void LinkedBinaryTree::addSubtreeMutator(mt19937& rng, const int maxDepth) {
  // get a position list of the nodes first

  // check to see if all operators have both a left and right child except for abs

  // if the operator is missing a left or right child, then add a subtree to that child

  PositionList pl = positions();

  for (int i = 0; i < pl.size(); i++) {
    // check to see if abs has its required operands
    if (*(pl[i]) == "abs" && pl[i].left().v == NULL) { // if the left node of abs operator is null, then we must add a tree here
      addLeftChild(pl[i]);
      randomExpressionTree(pl[i].left().v, maxDepth, rng); // create random tree here
    } else if (isOp(*(pl[i])) && pl[i].left().v == NULL) {
      addLeftChild(pl[i]);
      randomExpressionTree(pl[i].left().v, maxDepth, rng);
    } else if (isOp(*(pl[i])) && pl[i].right().v == NULL) {
      addRightChild(pl[i]);
      randomExpressionTree(pl[i].right().v, maxDepth, rng);
    }
  }
}
