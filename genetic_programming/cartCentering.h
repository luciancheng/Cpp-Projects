...
class cartCentering {
 protected:
  ...
  const int X = 0;
  const int V = 1;
  std::vector<double> state; // first item in the state vector is the x position and the second item is the velocity
 ...
 public:
  ...
  double update(const int& action, bool animate = false) {
    double force = action < 0 ? -FORCE_MAG : FORCE_MAG; // chooses which direction the rocket should go based on if the action is > or < than 0
    double acc_t = force / MASSCART;

    state[X] += TAU * state[V]; // updates the state of the car
    state[V] += TAU * acc_t; // calcualtes the velocity of the cart
    state[V] = bound(state[V], -MAX_V, MAX_V);  // bounds it within a range
    step++;
